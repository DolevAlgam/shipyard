    ...
    async def _process_topic(self, topic: str, state) -> None:
        ...
        while not topic_complete and follow_up_count < self.max_follow_ups:
            ...
            if follow_up_count == 0:
                ...
                # Use a natural conversation starter, not an instruction
                chat_history = self.state_manager.get_chat_history(self.pillar_name)
                agent_response = await self.client.call_agent(
                    system_prompt=enhanced_system_prompt,
                    user_message="Hi! I'm ready to answer your questions.",  # Natural conversation starter
                ...
                # For follow-ups, the user's actual response is already in chat history
                # We just need to prompt for a follow-up
                chat_history = self.state_manager.get_chat_history(self.pillar_name)
                agent_response = await self.client.call_agent(
                    system_prompt=enhanced_system_prompt,
                    user_message="Could you ask me a follow-up question about this?",  # Natural follow-up request
                    chat_history=chat_history,
                    model=self.model,
                    effort=self.effort,
                    reasoning_summary=self.reasoning_summary
